...
export type ChatObservationPayload = {
  platform: string;
  site?: string;
  shopName: string;
...
async function resolvePlatformAndShop(payload: ChatObservationPayload) {
...
export async function observeChat(payload: ChatObservationPayload) {
  ...
  if (existing) return { observation: existing, alerts: [], duplicated: true };
