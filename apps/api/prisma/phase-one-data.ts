...
type SetupOptions = {
  ...
  overwriteCollectorStatus?: boolean;
};

async function ensureUser(name: string, role: Role, languageSkills: string) {
  const existing = await prisma.user.findFirst({ where: { name } });
  if (existing) return existing;
  return prisma.user.create({ data: { name, role, languageSkills } });
}

export async function applyPhaseOneSetup(options: SetupOptions = {}) {
  ...
  const alertRules = [
    { name: "P0 立即升级", priority: "P0" as const, firstSlaMinutes: 1, escalateMinutes: 3, channels: "声音,弹窗,老板提醒" },
    { name: "P1 强提醒", priority: "P1" as const, firstSlaMinutes: 3, escalateMinutes: 5, channels: "声音,弹窗,客服提醒" },
    { name: "P2 普通提醒", priority: "P2" as const, firstSlaMinutes: 10, escalateMinutes: 15, channels: "红点,普通提醒" }
  ];
