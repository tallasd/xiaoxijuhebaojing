...
import { ImapFlow, type MailboxObject, type MessageAddressObject } from "imapflow";
...
type MailAccount = z.infer<typeof mailAccountSchema>;
...
const bodyFetchDisabledPlatforms = new Set(
  env.BODY_FETCH_DISABLED_PLATFORMS.split(",")
...
function loadState(): CollectorState {
  try {
    if (!fs.existsSync(stateFilePath)) return { accounts: {} };
    const parsed = JSON.parse(fs.readFileSync(stateFilePath, "utf8")) as CollectorState;
...
function normalizeState(state: CollectorState): CollectorState {
  const accounts: CollectorState["accounts"] = {};
...
function getAccountState(account: MailAccount): AccountState {
...
function updateAccountState(account: MailAccount, patch: Partial<AccountState>) {
  const state = getAccountState(account);
  Object.assign(state, patch, { updatedAt: new Date().toISOString() });
  saveState();
}

function detectPlatform(fromText: string, subjectText: string, account: MailAccount) {
  const from = fromText.toLowerCase();
  const subject = subjectText.toLowerCase();
  if (from.includes("etsy") || subject.includes("etsy")) return null;
...
function extractFirstLink(html?: string | false, text?: string) {
...
function isFetchVolumeLimit(error: unknown) {
  return /fetch volume limit exceed/i.test(describeError(error));
}

function formatAddresses(addresses?: MessageAddressObject[]) {
  return (
    addresses
...
function shouldFetchBody(rule: PlatformRule | null | undefined) {
  if (!env.FETCH_BODY_ENABLED) return false;
  if (!rule) return true;
...
async function heartbeat(account: MailAccount, message: string, status = "在线") {
...
async function initializeMailboxState(account: MailAccount, highestUid: number) {
  const state = getAccountState(account);
  if (state.initialized) return;

  const lastUid =
    env.INITIAL_SYNC_MODE === "latest" ? Math.max(0, highestUid - env.INITIAL_LOOKBACK) : Math.max(0, highestUid);
...
async function resolveHighestUid(client: ImapFlow, account: MailAccount, mailbox: MailboxObject) {
  const uidNext = Number(mailbox.uidNext);
  if (Number.isFinite(uidNext) && uidNext > 1) return Math.floor(uidNext - 1);

  try {
    const status = await client.status(account.mailbox, { uidNext: true });
    const statusUidNext = Number(status.uidNext);
    if (Number.isFinite(statusUidNext) && statusUidNext > 1) return Math.floor(statusUidNext - 1);
  } catch (error) {
    console.warn(`Unable to read UIDNEXT for ${account.name}: ${String(error)}`);
  }

  try {
    if (Number(mailbox.exists) <= 0) return 0;
    const latest = await client.fetchOne("*", { uid: true });
    const latestUid = latest ? Number(latest.uid) : 0;
    if (Number.isFinite(latestUid) && latestUid > 0) return Math.floor(latestUid);
  } catch (error) {
...
async function ingestMessage(client: ImapFlow, account: MailAccount, uid: number, envelopeFrom: string, envelopeSubject: string) {
  const envelopeRule = detectPlatform(envelopeFrom, envelopeSubject, account);
  if (!envelopeRule && !env.INGEST_UNKNOWN) {
    return "skipped";
  }

  let parsedData: ParsedEmailForIngest = {
  ...
  try {
    ...
    if (fullMessage && fullMessage.source) {
      const parsed = (await simpleParser(fullMessage.source)) as ParsedMail;
...
async function pollOnce(client: ImapFlow, account: MailAccount) {
  ...
  const state = getAccountState(account);
  if (highestUid <= state.lastUid) return { ingested: 0, skipped: 0, errors: 0, remaining: 0 };
...
async function runAccount(account: MailAccount) {
  for (;;) {
    ...
    try {
      ...
      const runPoll = async (reason: string) => {
        ...
        try {
          ...
          while (currentReason) {
            ...
            await heartbeat(
              account,
              `邮箱 ${account.name} IMAP 正常，${currentReason}：入库 ${result.ingested} 封，跳过 ${result.skipped} 封，异常 ${result.errors} 封，待处理 ${result.remaining} 封。`
            );
          ...
          if (isFetchVolumeLimit(error)) throw error;
        } finally {
          polling = false;
        }
      };

      const schedulePushPoll = (reason: string) => {
        if (!env.IMAP_PUSH_ENABLED) return;
        if (pushTimer) windowClearTimeout(pushTimer);
      ...
      client.on("exists", (data) => {
        if (data.path !== account.mailbox) return;
      ...
      try {
        await heartbeat(
          account,
          isFetchVolumeLimit(error)
            ? `邮箱 ${account.name} 网易 IMAP 流量超限，${env.LIMIT_BACKOFF_MINUTES} 分钟后再轻量检测，避免反复触发限制。`
            : `邮箱 ${account.name} 连接异常，10秒后重连：${message}`,
          isFetchVolumeLimit(error) ? "限流" : "异常"
        );
      } catch {}
    } finally {
...
async function main() {
  ...
  console.log(
    `Safe IMAP mode: push=${env.IMAP_PUSH_ENABLED}, bodyFetch=${env.FETCH_BODY_ENABLED}, bodyFetchDisabled=${env.BODY_FETCH_DISABLED_PLATFORMS}, ${env.POLL_SECONDS}s fallback poll, ${env.MAX_MESSAGES_PER_POLL} message(s) per account per poll.`
  );
