...
async function processEnvelope(raw) {
    ...
    if (envelope.kind === "message_ingest") {
        await post("/api/messages/ingest", {
            ...envelope.payload,
            sourceExternalId: typeof envelope.payload.sourceExternalId === "string"
...
async function pollOnce(client, seenUids) {
    ...
    try {
        for await (const msg of client.fetch({ seen: false }, { uid: true, envelope: true, source: true })) {
            ...
            if (!parsed.subject?.includes("[客服中台]"))
                continue;
